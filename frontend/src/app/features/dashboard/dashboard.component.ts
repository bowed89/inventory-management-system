...
import { ApiService } from '../../core/services/api.service'; // Service to interact with API
...
export class DashboardComponent {
  ...
  loadTransactions(): void {
    this.apiService.getAllTransactions('').subscribe((data) => {
      this.transactions = data.data ?? []; // Store transactions data
  ...
  loadMonthlyData(): void {
    ...
    this.apiService
      ...
      .subscribe((data) => {
        this.transactions = data.data ?? []; // Store transactions for the selected month
        this.processChartData(); // Process the overall data for charts
        this.processMonthlyData(data.data ?? []); // Process the data for the daily chart
      });
  }
